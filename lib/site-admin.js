...
// Replace a request's details — only if it was filed in this conversation and the
// developer hasn't started on it. The status filter sits in the UPDATE itself, so a
// request moved to in_progress a moment earlier can't be changed underneath her.
// Returns the updated row, or null if it wasn't amendable.
async function amendChangeRequest({ requestNo, conversationId, email, fields }) {
  const rows = await request(
    `site_change_requests?site=${eq(SITE)}&request_no=${eq(requestNo)}` +
      `&conversation_id=${eq(conversationId)}&requested_by=${eq(email)}&status=eq.new`,
    {
      method: "PATCH",
      headers: { Prefer: "return=representation" },
...
// Why an amend was refused, in words Claude can pass on to the owner.
async function whyNotAmendable({ requestNo, conversationId }) {
  const rows = await request(
    `site_change_requests?site=${eq(SITE)}&request_no=${eq(requestNo)}&select=status,conversation_id`,
    { method: "GET" }
  );
  if (!rows.length || rows[0].conversation_id !== conversationId) {
    return `Request #${requestNo} wasn't filed in this conversation, so it can't be corrected here. File a new request that says what should change instead.`;
  }
  return `Louise has already started on request #${requestNo}, so it can't be changed here. File a new request describing the correction and mention #${requestNo} in it.`;
}
