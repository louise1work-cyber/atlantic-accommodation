 ...
 * files it with submit_change_request — which writes a row and emails Nimbus Design —
 * or corrects one it already filed with amend_change_request (api/admin/[action].js
 * supplies both handlers). Stage 2 of the
...
If you or the owner spot a mistake in a request already filed in this conversation, or they change their mind about it, correct it with amend_change_request rather than filing another request.
...
// Tool enums are plain ASCII slugs, mapped to display names on the server. The first
// live test (2026-09-16) filed a Seaview request against the Apartment, then its own
// correction against the Beach Cottage: with strict tool use the model never once
// produced the option "Atlantic Seaview – Dolphin Beach", the only value containing a
// non-ASCII character (the en dash). Slugs sidestep that, and match lib/properties.js.
const PROPERTY_SLUGS = ["beach-cottage", "apartment", "seaview-dolphin-beach", "none"];
const propertyName = (slug) => (slug !== "none" && PROPERTIES[slug]) || null;

// The fields that describe one change — shared by filing and amending.
const REQUEST_FIELDS = {
  title: { type: "string", description: "Short summary, e.g. \"Update Beach Cottage amenities list\"." },
  property: {
    type: "string",
    enum: PROPERTY_SLUGS,
    description: "beach-cottage = Atlantic Beach Cottage; apartment = Atlantic Apartment; seaview-dolphin-beach = Atlantic Seaview (Dolphin Beach); none = not about one home."
  },
...
const SUBMIT_TOOL = {
  ...
  input_schema: {
    ...
    required: Object.keys(REQUEST_FIELDS),
...
const AMEND_TOOL = {
  ...
  description:
    "Correct a request already filed in this conversation by replacing all of its details — use this instead of filing a second request when you or the owner spot a mistake, or they change their mind. Only possible until Louise starts work on it; if it fails, explain that to the owner.",
  strict: true,
  input_schema: {
    type: "object",
    additionalProperties: false,
    required: ["request_no", ...Object.keys(REQUEST_FIELDS)],
    properties: {
      request_no: { type: "integer", description: "The number of the request to correct." },
      ...REQUEST_FIELDS
    }
  }
};

const TOOLS = [SUBMIT_TOOL, AMEND_TOOL];
 ...
 *
 * `handlers` maps each tool name to an async function that does the work and resolves
 * to the object handed back to Claude as the tool result (and reported to the page);
 * if it throws, Claude is told why so it can tell the owner.
 */
async function runTurn({ messages, siteText, handlers }) {
  ...
  for (let round = 0; round < MAX_TOOL_ROUNDS; round++) {
    ...
    for (const block of response.content) {
      ...
      try {
        ...
        console.error(`${block.name} failed:`, err.message);
...
module.exports = { configured, runTurn, propertyName, MODEL, Anthropic };
