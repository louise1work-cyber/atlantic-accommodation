 ...
 *
 * lib/ical.js WRITES our direct bookings out for Airbnb and Booking.com to
 * import. This file READS a channel's own export back in, so this site knows
 * which dates that channel has already taken. Without it the site has no idea
 * a guest is enquiring about a week that was booked on Airbnb three days ago.
 * Platform-agnostic on purpose: Airbnb's and Booking.com's exports are both
 * plain RFC 5545 calendars, so one reader serves both — see
 * api/availability/[property].js for how each channel's URL is wired in.
 *
 * These exports give all-day VEVENTs and nothing else useful: no guest, no
...
/**
 * Fetch and parse one channel's export URL.
 *
 * Throws on network failure, timeout, or a non-200 — the caller decides how to
 * degrade, because "we couldn't reach the channel" must never be rendered to a
 * guest as "these dates are free". `label` (e.g. "Airbnb", "Booking.com") is
 * only used to make that thrown error readable in logs.
 */
async function fetchCalendar(url, timeoutMs, label) {
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), timeoutMs || 8000);
  const name = label || "Calendar source";
  ...
  try {
    ...
    if (!/BEGIN:VCALENDAR/i.test(text)) {
      // Some channels serve an HTML error page rather than a 404 when an
      // export URL has been revoked, so a 200 alone isn't proof we got a
      // calendar.
      throw new Error(`${name} response was not an iCalendar document`);
    }
