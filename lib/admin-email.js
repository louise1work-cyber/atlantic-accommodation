...
// `photos` is [{ name, url }] with time-limited signed links.
async function sendChangeRequest(request, photos) {
  ...
  await send({
    ...
    subject: `Change request #${request.request_no} – Atlantic Accommodation – ${request.title}`,
    html: shell(`
