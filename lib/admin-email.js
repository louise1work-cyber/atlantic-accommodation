...
const OWNER_ADDRESS = process.env.ENQUIRY_TO || "info@atlanticaccommodation.co.za";
...
async function sendLoginLink(email, url, minutes) {
  // Deliberately no reply_to: xneelo's SpamAssassin scores FROM_NOT_REPLYTO at +0.5
  // when they differ (seen on the junked 2026-09-22 message), and the address to write
  // to is in the text instead.
  await send({
    from: ADMIN_FROM,
    to: [email],
    ...
    html: shell(`
      ...
      <p style="margin:0 0 20px">
        <a href="${esc(url)}" style="display:inline-block;background:#566C65;color:#F4F8F6;text-decoration:none;padding:13px 26px;border-radius:999px;font:500 13px/1 -apple-system,Segoe UI,sans-serif;letter-spacing:.08em;text-transform:uppercase">Sign in</a>
      </p>
